...
function main(
    ...
    buildUnitRows(unitCount, rows, updatedAt, snapshotDate, warnings);
    ...
    buildFinanceRows(finance, rows, updatedAt, snapshotDate, warnings);
    const financeAggregateCount = rows.length - operationsAggregateCount - unitAggregateCount - agencyAggregateCount;
...
function createTestRows(updatedAt: string, snapshotDate: string): ExportRow[] {
    ...
    add(2026, 8, "Monthly Unit Progress", "Booked Total", 440, "number", 450);
...
function addFinanceValues(a: Aggregate, record: InputRecord): void {
    addValue(a, "actualRevenue", -numberValue(record, "[Actual]"));
    addValue(a, "outlookRevenue", -numberValue(record, "[Forecast]"));
    addValue(a, "budgetRevenue", -numberValue(record, "[Budget]"));
